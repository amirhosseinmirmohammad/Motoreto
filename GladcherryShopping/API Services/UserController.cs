...
namespace GladcherryShopping.API_Services
{
    public class UserController : ApiController
    {
        ...
        string BaseUrl = "https://motoreto.ir/";

        [AcceptVerbs("POST")]
        [Route("User/Scan")]
        public IHttpActionResult UserScan(HttpRequestMessage requset)
        {

            #region Request
            var body = requset.Content;
            var jsonContent = body.ReadAsStringAsync().Result;
            UserScan requestParams = JsonConvert.DeserializeObject<UserScan>(jsonContent);
            #endregion Request

            #region Validation
            if (string.IsNullOrWhiteSpace(requestParams.QrCode))
                return Ok(new { Status = 0, Text = "کیو آر کد را وارد نمایید ." });
            if (string.IsNullOrWhiteSpace(requestParams.SerialNumber))
                return Ok(new { Status = 0, Text = "لطفا سریال نامبر را وارد نمایید ." });
            #endregion Validation

            #region Find
            QrEntity model = db.QrEntities.Where(current => current.QrCode == requestParams.QrCode && current.SerialNumber == requestParams.SerialNumber).FirstOrDefault();
            if (model != null)
            {
                return Ok(new { Status = 1, Text = "کالا اصیل است ." });
            }
            else
            {
                return Ok(new { Status = 1, Text = "کالا اصیل نیست ." });
            }
            #endregion Find
        }
        ...
        [AcceptVerbs("POST")]
        [Route("Admin/Scan")]
        public IHttpActionResult ScanCode(HttpRequestMessage requset)
        {

            #region Request
            var body = requset.Content;
            var jsonContent = body.ReadAsStringAsync().Result;
            ScanCode requestParams = JsonConvert.DeserializeObject<ScanCode>(jsonContent);
            #endregion Request
            ...
            if (string.IsNullOrWhiteSpace(requestParams.SerialNumber))
                return Ok(new { Status = 0, Text = "لطفا سریال نامبر را وارد نمایید ." });
            if (requestParams.ProductId == null)
                return Ok(new { Status = 0, Text = "لطفا شماره محصول را وارد نمایید ." });
            Product pro = db.Products.Find(requestParams.ProductId);
            if (pro == null)
                return Ok(new { Status = 0, Text = "محصول مورد نظر یافت نشد ." });
            #endregion Validation

            #region Code
            QrEntity model = new QrEntity
            {
            ...
            try
            {
                db.QrEntities.Add(model);
                db.SaveChanges();
                return Ok(new { Status = 1, Text = "کیو آر کد مورد نظر شما با موفقیت در سامانه ثبت شد ." });
            }
            catch (Exception)
            {
        ...
        public IHttpActionResult AdminLogin(HttpRequestMessage requset)
        {

            #region Request
            var body = requset.Content;
            var jsonContent = body.ReadAsStringAsync().Result;
            AdminLogin requestParams = JsonConvert.DeserializeObject<AdminLogin>(jsonContent);
            #endregion Request

            #region Validation
            if (string.IsNullOrWhiteSpace(requestParams.UserName))
                return Ok(new { Status = 0, Text = "نام کاربری را وارد نمایید ." });
            if (string.IsNullOrWhiteSpace(requestParams.Password))
                return Ok(new { Status = 0, Text = "رمز عبور را وارد نمایید ." });
            #endregion Validation
            ...
            var result = UserManager.Find(requestParams.UserName, requestParams.Password);
            if (result != null)
            {
                ApplicationUser user = context.Users.Where(current => current.UserName == requestParams.UserName).Include(current => current.Roles).FirstOrDefault();
                if (user.Roles.FirstOrDefault().RoleId == adminRole)
                {
                    return Ok(new { Status = 1, Text = "ورود شما با موفقیت انجام شد ." });
                }
                else
                {
                    return Ok(new { Status = 0, Text = "اطلاعات ورود شما اشتباه است ." });
                }
            }
            else
            {
        ...
        public IHttpActionResult GenerateVerificationCode(HttpRequestMessage requset)
        {
            ...
            if (string.IsNullOrWhiteSpace(requestParams.FirstName))
                return Ok(new { Status = 0, Text = "لطفا نام خود را وارد نمایید ." });
            if (string.IsNullOrWhiteSpace(requestParams.LastName))
                return Ok(new { Status = 0, Text = "لطفا نام خانوادگی خود را وارد نمایید ." });
            if (string.IsNullOrWhiteSpace(requestParams.BirthDate))
                return Ok(new { Status = 0, Text = "لطفا تاریخ تولد خود را وارد نمایید ." });
            ...
            ApplicationUser user = new ApplicationUser
            {
                ...
                Credit = 0,
                IsActive = true,
                BirthDate = FunctionsHelper.ConvertToGregorian(requestParams.BirthDate),
            ...
            try
            {
                ...
                {
                    //کد معرفی کاربر جدید
                    user.IntroCodeOptional = FriendUser.AccessCode;
            ...
            if (UserVerificationCode != null)
            {
                ...
                if (UserVerificationCode == Code)
                {
                    ...
                    if (IsRegistered == true)
                    {
                        ...
                        #region IntroCode
                        if (!string.IsNullOrWhiteSpace(users.FirstOrDefault().IntroCodeOptional))
                        {
                            users.FirstOrDefault().IntroCode = users.FirstOrDefault().IntroCodeOptional;
                            ApplicationUser FriendUser = db.Users.Where(current => current.AccessCode == users.FirstOrDefault().IntroCodeOptional).FirstOrDefault();
        ...
        public IHttpActionResult ResendVerificationCode(string PhoneNumber)
        {
            ...
            if (application != null && application.FromNumber != null)
            {
                ...
                //sms.ir
                string SMSDescription = "کد مجدد تایید شما در موتورتو : " + randomNumber;
                bool smsResult = UltraFastSms(PhoneNumber, randomNumber.ToString());
                if (smsResult == false)
                    return Ok(new { Status = 0, Text = "سامانه پیامکی با خطا رو به رو است لطفا مجدد تلاش فرمایید ." });
        ...
        public IHttpActionResult UserInformation(string UserId, string GuId)
        {
            ...
            if (User.State != null && User.City != null)
            {
                return Ok(new
                {
                    ...
                    PersianBirh = FunctionsHelper.GetPersianDateTime((DateTime)User.BirthDate, false, false),
                    cityName = User.City.Name ?? "",
                    stateName = User.State.Name ?? "",
                    Menu = "https://homekar.com/images/Menu.jpg",
                    EditProfile = "https://homekar.com/images/EditProfile.jpg",
                    Messages = "https://homekar.com/images/Messages.png",
                    AboutUs = "https://homekar.com/images/AboutUs.png",
                    Logout = "https://homekar.com/images/Logout.png",
                    Payments = "https://homekar.com/images/Payments.png",
                    Profile = "https://homekar.com/images/Profile.png",
                    Rate = "https://homekar.com/images/Rate.png",
                    Rules = "https://homekar.com/images/Rules.png",
                    SelectedAdresses = "https://homekar.com/images/SelectedAdresses.png",
                    Support = "https://homekar.com/images/Support.png",
        ...
        public IHttpActionResult EditUser(HttpRequestMessage requset)
        {
            ...
            if (!string.IsNullOrEmpty(requestParams.BirthDate))
            {
                EdittedUser.BirthDate = FunctionsHelper.ConvertToGregorian(requestParams.BirthDate);
            }
        ...
        public IHttpActionResult OrderPost()
        {
            ...
            try
            {
                ...
                return Ok(new { Status = 1, path = "http://motoreto.ir/Content/Activities/" + imageName, Text = "عکس مورد نظر با موفقیت آپلود شد ." });
            }
            catch (Exception)
            {
        ...
        public IHttpActionResult UserPost(string UserId)
        {
            ...
            try
            {
                ...
                byte[] imageBytes = Convert.FromBase64String(x);
                System.IO.File.WriteAllBytes(imgPath, imageBytes);
                User.ProfileImage = "http://motoreto.ir/Content/UserProfiles/" + imageName;
                db.Entry(User).State = EntityState.Modified;
                db.SaveChanges();
                return Ok(new { Status = 1, path = "http://motoreto.ir/Content/UserProfiles/" + imageName, Text = "عکس مورد نظر با موفقیت آپلود شد ." });
            }
            catch (Exception)
            {
                return Ok(new { Status = 0, path = string.Empty, Text = "خطایی رخ داده است لطفا مجدد تلاش فرمایید ." });
        ...
        [AcceptVerbs("POST")]
        [Route("Products/List")]
        public IHttpActionResult ProductsList(string FromPrice, string ToPrice, string Latitude, string Longitude, string EntryText, string DiscountPercent, string SortBy, int? CategoryId, string UserId, string GuId, HttpRequestMessage requset)
        {
           ...
           .Where(currrent => currrent.AppSmallImage != null);
            if (CategoryId != null)
            {
            ...
            {
                byte percent = Convert.ToByte(DiscountPercent);
                finalProducts = finalProducts.Where(current => current.DiscountPercent >= percent);
            }
            if (!string.IsNullOrEmpty(EntryText))
            {
                finalProducts = finalProducts.Where(current => current.PersianName.Contains(EntryText) || current.Description.Contains(EntryText) || current.EnglishName.Contains(EntryText));
            }
            ...
            if (!string.IsNullOrEmpty(SortBy))
            {
                ...
                {
                    case "1":
                        foreach (var pr in finalProducts.OrderByDescending(current => current.CreateDate).ToList())
                        {
                            ProductList.Add(new
                            {
                                Id = pr.Id.ToString(),
        ...
        public IHttpActionResult Main(string UserId, string GuId, string PlayerId)
        {
           ...
           .Include(current => current.category)
           .OrderByDescending(current => current.CreateDate).Where(currrent => currrent.AppSmallImage != null);

            IQueryable<Product> specialProducts = db.Products
           .Include(current => current.category)
           .OrderByDescending(current => current.CreateDate).Where(currrent => currrent.AppSmallImage != null && currrent.IsSpecial == true);

            #region FindProducts
            List<dynamic> WonderProduct = new List<dynamic>();
            string day = string.Empty;
            string hour = string.Empty;
            string min = string.Empty;
            Product wonder = db.Products.Where(current => current.Stock > 0 && current.AppLargeImage != null && current.IsWonderful == true && current.Day != null && current.Minute != null && current.Hour != null).FirstOrDefault();
            if (wonder != null)
            {
                ...
                {
                    TimeSpan span = (wonder.WonderDate.Value.AddDays(Convert.ToDouble(wonder.Day)).AddHours(Convert.ToDouble(wonder.Hour)).AddMinutes(Convert.ToDouble(wonder.Minute)) - DateTime.Now);
                    day = span.Days.ToString();
                    hour = span.Hours.ToString();
                    min = span.Minutes.ToString();
                }
                if (!string.IsNullOrEmpty(day) && !string.IsNullOrEmpty(hour) && !string.IsNullOrEmpty(min))
                {
                    WonderProduct.Add(new
                    {
                        Id = wonder.Id.ToString(),
                        PersianName = wonder.PersianName,
                        UnitPrice = wonder.UnitPrice,
                        PriceWithDiscount = wonder.UnitPrice - (wonder.UnitPrice) * (wonder.DiscountPercent) / 100,
                        Discount = wonder.DiscountPercent,
                        Image = BaseUrl + wonder.AppLargeImage,
                        Stock = wonder.Stock,
                        Day = day,
                        Hour = hour,
                        Minute = min
            ...
            foreach (var pr in newestProducts.ToList())
            {
            ...
            foreach (var pr2 in specialProducts.ToList())
            {
            ...
            var sliders = db.Sliders.Where(current => current.IsApp == true);
            if (sliders != null && sliders.Count() > 0)
            {
                foreach (var item in sliders.Where(current => current.IsApp == true).OrderByDescending(current => current.LastDate).Take(6).ToList())
                {
                    Sliders.Add(new
                    {
                        Id = item.Id.ToString(),
            ...
            else
            {
                ...
                {
                    Id = "",
                    Image = BaseUrl + "images/sadr_logo.png"
                });
            }
            return Ok(new { Status = 1, NewestProducts = NewProductList, NewestProductsCount = NewProductList.Count, SpecialProducts = SpecialProductList, SpecialProductsCount = SpecialProductList.Count, Sliders = Sliders, SlidersCount = Sliders.Count, BasketCount = BasketCount(UserId, GuId), WonderProduct = WonderProduct });
            #endregion FindProducts
        }
        ...
        public IHttpActionResult ProductDetails(long? ProductId, string UserId, string GuId)
        {
            ...
            if (ProductId == null)
                return Ok(new { Status = 0, Text = "لطفا شناسه محصول را وارد نمایید ." });
            Product product = db.Products.Where(current => current.Id == ProductId).Include(current => current.category).Include(current => current.RelatedProducts).Include(current => current.Images).FirstOrDefault();
            if (product == null)
        ...
        public IHttpActionResult GetActivity(string UserId, string GuId)
        {
            ...
            if (string.IsNullOrEmpty(UserId))
                return Ok(new { Status = 0, Text = "لطفا شناسه کاربری را وارد نمایید ." });
            var Orders = db.Orders.Where(current => current.UserId == UserId).Include(current => current.Products).OrderByDescending(current => current.OrderDate).Take(10).ToList();
            if (Orders.Count == 0)
            ...
            foreach (var item in Orders)
            {
                ...
                {
                    Product pro = db.Products.Find(item.Products.FirstOrDefault().ProductId);
                    if (pro != null)
                    {
        ...
        public IHttpActionResult SubmitOrder(HttpRequestMessage requset)
        {
            ...
            if (requestParams.PaymentType == null || requestParams.PaymentType == 0)
                return Ok(new { Status = 0, Text = "لطفا روش ارسال را تعیین نمایید ." });
            ...
            newOrder.PaymentType = (byte)requestParams.PaymentType;
            ...
            if (BasketItems(requestParams.UserId, requestParams.GuId).Count > 0)
            {
                foreach (var item in BasketItems(requestParams.UserId, requestParams.GuId))
                {
                    ...
                    if (pro != null)
                    {
                        if (item.Count > pro.Stock)
                        {
                            return Ok(new { Status = 0, Text = "متاسفانه موجودی " + pro.PersianName + " با تعداد دلخواه شما به اتمام رسیده است ." });
                        }
                        if(item.Count < pro.Min)
                        {
                            return Ok(new { Status = 0, Text = "حداقل خرید " + pro.PersianName + " تعداد " + pro.Min + " است ." });
                        }
                ...
                if (user.Credit >= Convert.ToInt64(newOrder.TotalPrice))
                {
                    user.Credit -= Convert.ToInt64(newOrder.TotalPrice);

                    #region AddPayment
                    Payment payment = new Payment();
                    payment.UserId = newOrder.UserId;
                    payment.Amount = Convert.ToInt64(newOrder.TotalPrice);
                    payment.Status = 3;
                    payment.ActionType = 1;
                    payment.OrderId = newOrder.Id;
                    payment.Description = "پرداخت هزینه سفارش : " + newOrder.Id + " از کیف پول ";
                    payment.TrackingCode = newOrder.InvoiceNumber;
                    ...
                    db.Entry(newOrder).State = EntityState.Modified;
                    db.SaveChanges();
                    return Ok(new { Status = 1, Text = "کاربر گرامی سفارش شما ثبت و هزینه آن از کیف پول شما کسر شد .", Online = "False" });
                }
                else
                {
                    //tejarat
                    ...
                    var callbackurl = "https://motoreto.ir/Home/VerifyOrder?order=" + newOrder.Id;
                    Random random = new Random();
                    var randomNumber = random.Next(10000, 99999);
                    if (newOrder.PaymentType == 1)
                    {
                        price = Convert.ToInt32(app.Transferring) - (Convert.ToInt32(user.Credit));
                    }
                    else if (newOrder.PaymentType == 2)
                    {
                    ...
                    if (responseData.Status == 0)
                    {
                        ...
                        return Ok(new { Status = 1, Text = "https://pec.shaparak.ir/NewIPG/?Token=" + token, Online = "True" });
                    }
                    if (responseData.Status == -112)
                    {
                        return Ok(new { Status = 0, Text = "سفارش شما تکراری است لطفا درخواست فاکتور جدید از شرکت نمایید ." });
                    }
                    else
                    {
                        return Ok(new { Status = 0, Text = "امکان اتصال به درگاه بانک وجود ندارد ." + responseData.Status });
                    }
                    #endregion Tejarat
        ...
        public IHttpActionResult GetWithStock(string q = "")
        {
            try
            {
                var query = db.Products.Where(p => p.Stock > 0);
